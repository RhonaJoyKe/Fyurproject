...
app.config['SQLALCHEMY_DATABASE_URI']='postgresql://postgres:12345@localhost:5432/fyurproject'
...
def venues():
  ...
  for city in cities:
    ...
    for venue in venue:
      upcoming_shows_count = db.session.query(Shows).filter_by(venue_id=venue.id).count()
      venues.append({
        'city':city.city,
        'state':city.state,
        "venues":[{
...
def show_venue(venue_id):
  ...
  now = datetime.now()
  data = venue.__dict__
  shows = db.session.query(Shows).filter_by(venue_id=venue_id)
  past_shows = shows.filter(Shows.start_time < now).all()
  upcoming_shows =  shows.filter(Shows.start_time >= now).all()
...
def create_venue_submission():
  ...
  try:
    ...
    if talent == 'y':
      ...
      talent = False
    venue_data = Venue(name=name,city=city,state=state,address=address,phone=phone,genres=genres,image_link=image_link,facebook_link=facebook_link,website_link=website_link,talent=seeking_talent,description=seeking_description)
...
def show_artist(artist_id):
  ...
  shows = db.session.query(Shows).filter_by(artist_id=artist_id)
  past_shows = shows.filter(Shows.start_time < now).all()
  upcoming_shows =  shows.filter(Shows.start_time >= now).all()
  upcoming_shows_count = len(upcoming_shows)
  past_shows_count = len(past_shows)
...
def create_show_submission():
  ...
  try:
    ...
    show_data = Shows(artist_id=artist,venue_id=venue,start_time=date)
